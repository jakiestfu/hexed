...
describe("WindowManager", () => {
  ...
  describe("getWindow", () => {
    it("should load and cache a window", async () => {
      ...
      expect(readFn).toHaveBeenCalledWith(
        0,
        windowSize + DEFAULT_WINDOW_CONFIG.overlap
      )
    })
    ...
    it("should extract requested range from cached window", async () => {
      const windowSize = DEFAULT_WINDOW_CONFIG.size
      const fullWindowData = createTestData(
        windowSize + DEFAULT_WINDOW_CONFIG.overlap
      )
      ...
      const offset = 100
      const length = 50
      const data = await manager.getWindow(
        fileId,
        offset,
        offset + length,
        readFn
    ...
    it("should handle window overlap", async () => {
      ...
      const readFn = vi
        .fn()
        .mockResolvedValue(createTestData(windowSize + overlap))
  ...
  describe("LRU cache eviction", () => {
    ...
    it("should evict oldest windows first", async () => {
      ...
      const readFn2 = vi
        .fn()
        .mockResolvedValue(createTestData(windowSize + 100))
  ...
  describe("setWindowSize", () => {
    it("should update window size for a file", async () => {
      ...
      const readFn = vi
        .fn()
        .mockResolvedValue(createTestData(newWindowSize + 100))
      ...
      expect(readFn).toHaveBeenCalledWith(
        0,
        newWindowSize + DEFAULT_WINDOW_CONFIG.overlap
      )
    })
